...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    const [stepsResult, progressResult, tierResult] = await Promise.all([
      pool.query('SELECT * FROM ladder_steps WHERE boss_question_id = $1 AND is_active = true ORDER BY step_number', [id]),
      pool.query('SELECT * FROM student_progress WHERE student_id = $1 AND boss_question_id = $2', [studentId, id]),
      pool.query('SELECT * FROM student_tier_progress WHERE student_id = $1 AND boss_question_id = $2', [studentId, id]),
    ]);

    const progress = progressResult.rows[0] || { current_step: 1, steps_completed: 0, progress_percentage: 0, is_completed: false };
    const tierState = tierResult.rows[0] || null;
    ...
    res.json({ success: true, boss_question: boss, steps, student_progress: progress, tier_state: tierState });
  } catch (err) {
